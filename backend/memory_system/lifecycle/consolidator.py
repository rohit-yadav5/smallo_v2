        ...
        for mem_b in consolidated[i + 1:]:
            ...
            if sim >= META_CONSOLIDATION_SIMILARITY_THRESHOLD:
                ...
                for src_id in (mem_a["id"], mem_b["id"]):
                    ...
                    """, (f"rel-meta-{src_id[:8]}", src_id, new_id, datetime.utcnow().isoformat()))
                conn.commit()
                conn.close()
...
def expire_old_consolidated_memories():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, importance_score, memory_type, created_at FROM memories
    ...
    expired_ids = []
    for row in rows:
        eff = calculate_effective_importance(row["importance_score"], row["memory_type"], row["created_at"])
        try:
            age_days = (datetime.utcnow() - datetime.fromisoformat(row["created_at"])).days
        except Exception:
            age_days = 0
        if (age_days > CONSOLIDATED_MEMORY_EXPIRY_DAYS and eff < CONSOLIDATED_MEMORY_EXPIRY_MIN_IMPORTANCE):
